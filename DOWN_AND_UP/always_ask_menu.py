...
def askq_callback(app, callback_query):
    ...
    if data == "link":
        ...
        if result.get('success'):
            ...
            main_response = messages.STREAM_LINKS_TITLE_MSG
            main_response += messages.STREAM_TITLE_MSG.format(title=title)
            if duration > 0:
        ...
        if success:
            ...
            try:
                ...
                if video_only_formats:
                    ...
                    caption += f"\n{messages.LIST_VIDEO_ONLY_FORMATS_MSG.format(formats=video_formats_text)}\n"
                ...
                if audio_only_formats:
                    ...
                    caption += f"{messages.ALWAYS_ASK_AUDIO_ONLY_FORMATS_MSG}: {audio_formats_text}\n"
...
def show_manual_quality_menu(app, callback_query):
    ...
    # {messages.ALWAYS_ASK_BEST_BUTTON_MSG} Quality
    if is_playlist and playlist_range:
        indices = list(range(playlist_range[0], playlist_range[1]+1))
        ...
        icon = "🚀" if (n_cached > 0 and not is_nsfw) else ("1⭐️" if (is_nsfw and is_private_chat) else "📹")
        postfix = f" ({n_cached}/{total})" if total > 1 else ""
        button_text = f"{icon}{messages.ALWAYS_ASK_BEST_BUTTON_MSG} Quality{postfix}"
    else:
        icon = "🚀" if ("best" in cached_qualities and not is_nsfw) else ("1⭐️" if (is_nsfw and is_private_chat) else "📹")
        button_text = f"{icon}{messages.ALWAYS_ASK_BEST_BUTTON_MSG} Quality"
...
def show_other_qualities_menu(app, callback_query, page=0):
    ...
    cap += f"\n<i>{messages.PAGE_NUMBER_MSG.format(page=page + 1)}</i>\n"
...
def create_cached_qualities_menu(app, message, url, tags, proc_msg, user_id, original_text, is_playlist, playlist_range):
    ...
    try:
        ...
        # Всегда добавляем {messages.ALWAYS_ASK_BEST_BUTTON_MSG} Quality
        quality_key = "best"
        if is_playlist and playlist_range:
            indices = list(range(playlist_range[0], playlist_range[1]+1))
            n_cached = get_cached_playlist_count(get_clean_playlist_url(url), quality_key, indices)
            ...
            icon = "🚀" if (n_cached > 0 and not is_nsfw) else ("1⭐️" if (is_nsfw and is_private_chat) else "📹")
            postfix = f" ({n_cached}/{total})" if total > 1 else ""
            button_text = f"{icon}{messages.ALWAYS_ASK_BEST_BUTTON_MSG}{postfix}"
        else:
            icon = "🚀" if (quality_key in cached_qualities and not is_nsfw) else ("1⭐️" if (is_nsfw and is_private_chat) else "📹")
            button_text = f"{icon}{messages.ALWAYS_ASK_BEST_BUTTON_MSG}"
        buttons.append(InlineKeyboardButton(button_text, callback_data=f"askq|{quality_key}"))
        ...
        try:
            if is_youtube_url(url):
                ...
                try:
                    ...
                    is_group = False
                if is_group:
                    action_buttons.append(InlineKeyboardButton(messages.ALWAYS_ASK_WATCH_BUTTON_MSG, url=piped_url))
                else:
                    wa = WebAppInfo(url=piped_url)
                    action_buttons.append(InlineKeyboardButton(messages.ALWAYS_ASK_WATCH_BUTTON_MSG, web_app=wa))
        except Exception as e:
...
def ask_quality_menu(app, message, url, tags, playlist_start_index=1, cb=None, download_dir=None):
    ...
    try:
        ...
        # Always add {messages.ALWAYS_ASK_BEST_BUTTON_MSG} Quality button
        quality_key = "best"
        if is_playlist and playlist_range:
            indices = list(range(playlist_range[0], playlist_range[1]+1))
            n_cached = get_cached_playlist_count(get_clean_playlist_url(url), quality_key, indices)
        ...
        action_buttons.append(InlineKeyboardButton(messages.ALWAYS_ASK_LINK_BUTTON_MSG, callback_data="askq|link"))
...
def askq_callback_logic(app, callback_query, data, original_message, url, tags_text, available_langs, proc_msg=None):
    ...
    if data == "best":
        try:
            ...
            pass
        # Use format with AVC codec and MP4 container priority for {messages.ALWAYS_ASK_BEST_BUTTON_MSG} quality
