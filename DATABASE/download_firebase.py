...
if not FIREBASE_CONFIG or not FIREBASE_USER or not FIREBASE_PASSWORD:
    print(safe_get_messages(None).DB_NOT_ALL_PARAMETERS_SET_MSG)
    sys.exit(1)
...
def download_firebase_dump():
    ...
    if requests is None or Session is None:
        print(safe_get_messages(None).DB_DEPENDENCY_NOT_AVAILABLE_MSG)
        return False
    ...
    try:
        print(safe_get_messages(None).DB_STARTING_FIREBASE_DUMP_MSG.format(datetime=datetime.now()))

        database_url = FIREBASE_CONFIG.get("databaseURL")
        if not database_url:
            print(safe_get_messages(None).DB_DATABASE_URL_NOT_SET_MSG)
            return False
        ...
        if not key:
            print(safe_get_messages(None).DB_API_KEY_NOT_SET_MSG)
            return False
        ...
        # Downloading data
        print(safe_get_messages(None).ALWAYS_ASK_DOWNLOADING_DATABASE_MSG)
        ...
        if data:
            ...
            print(safe_get_messages(None).DB_DATABASE_EMPTY_MSG)

        return True

    except Exception as e:
        print(safe_get_messages(None).DB_ERROR_DOWNLOADING_DUMP_MSG.format(error=e))
        return False
    finally:
