...
# -*- coding: utf-8 -*-
"""
Быстрый скрипт для удаления неуникальных строк из файла porn_keywords.txt
Оптимизированная версия с эффективным алгоритмом
"""
...
def remove_duplicate_keywords_fast(input_file: str, output_file: str = None) -> None:
    """
    Быстро удаляет строки, которые содержатся как полные слова в других строках.
    """
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        for line in sorted(unique_lines):  # Сортируем для консистентности
            f.write(line + '\n')
...
def main():
    """Основная функция"""
    if len(sys.argv) < 2:
        print("Использование: python remove_duplicate_keywords_fast.py <input_file> [output_file]")
        print("Пример: python remove_duplicate_keywords_fast.py TXT/porn_keywords.txt")
        sys.exit(1)
    
    input_file = sys.argv[1]
    output_file = sys.argv[2] if len(sys.argv) > 2 else None
    
    remove_duplicate_keywords_fast(input_file, output_file)
